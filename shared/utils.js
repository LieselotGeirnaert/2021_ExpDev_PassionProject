...
class Reticle extends THREE.Object3D {
  constructor() {
    ...
    this.loader.load("../assets/mars.glb", (gltf) => {
      this.add(gltf.scene);
    })
...
window.gltfLoader.load("../assets/mars.glb", function (gltf) {
...
window.DemoUtils = {
  ...
  /**
   * Creates a THREE.Scene containing planets all over the scene.
   *
   * @return {THREE.Scene}
   */
  createPlanetScene() {
    ...
    const radius = 0.1; // ui: radius
    const widthSegments = 32; // ui: widthSegments
    const heightSegments = 16; // ui: heightSegments
    ...
    for (let i = 0; i < ROW_COUNT; i++) {
      for (let j = 0; j < ROW_COUNT; j++) {
        for (let k = 0; k < ROW_COUNT; k++) {
          ...
          sphere.position.set(i - HALF, j - HALF, k - HALF);
          sphere.position.multiplyScalar(SPREAD);
          scene.add(sphere);
        }
      }
    }
  ...
  createSphereScene() {
    ...
    loader.load("../assets/mars.jpeg", (texture) => {
